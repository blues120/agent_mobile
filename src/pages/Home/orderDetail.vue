<template>
  <div class="order-detail">
    ...
    <section class="orderbot"  v-if="orderObject.closed==0">
      ...
      <button class="buyagain" v-if="orderObject.orderStatus==4" @click="bugAgain(orderObject.id,orderObject.orderClass)">再次购买</button>
    </section>
    <section class="orderbot"  v-else>
      <button  @click="deleteOrder(orderObject.id)">删除订单</button>
      <button class="buyagain"  @click="bugAgain(orderObject.id,orderObject.orderClass)">再次购买</button>
    </section>
    <div v-transfer-dom>
