<template>
    <div class="go-settlement">
      ...
      <!--地址栏信息部分-->
      <section class="address" v-if="cartType==1" @click="chooseaddress(addressInfoObj?addressInfoObj.id:'')">
      ...
      <section  class="goodsbox">
        <div class="eachGoods" v-for="(list,index) in templeCarList" :key="index">
          ...
          <div class="goodsdetail">
              ...
              <p style="justify-content: flex-end"><span class="goodsnum">X{{list.totalCartProductCount}}</span></p>
          </div>
        </div>
      </section>
      <!--底部结算-->
      <section class="footersettlement">
        <div class="footleft">
          <span>合计： </span><span class="dd-price">¥ 0.00</span>
        </div>
        <div class="surebtn" @click="goSureOrder">提交订单</div>
      </section>
      <div v-transfer-dom>
...
export default {
  ...
  methods: {
    ...
    goSureOrder () { // 先提交订单，后请求支付方式，选择支付方式支付
      ...
      this.$http.post('myInventory/deliveryOrderSubmit', objpara,
        {
          headers: {
        ...
        this.czorderId = res.data.data
        this.payObj = {
          free: true, // 需要支付的金额是0
          CloudGoodschange: 2, // 云库存付款类别 1 是换货支付  2 是 提货支付
          showWaring: false, // 是否显示离开支付页面默认不显示
    ...
    confirmPay () { // 付款0 操作完成~后调用
      var params = {
        orderId: this.czorderId
      }
      params = Qs.stringify(params)
      this.$http.post('placeOrders/payment', params).then((res) => {
        if (res) {
          this.$vux.toast.text('操作成功!', 'middle')
          this.$router.replace({name: 'paySuccess', query: {orderId: this.czorderId}})
