        ...
        <cube-slide
          ...
          <cube-slide-item  v-for="(goodsListArry,inde) in AllGoodsListArry" :key="inde" class="swiperboxs">
            <cube-scroll :ref="'scroll'+inde" :options="scrollOptions"  :data="goodsListArry"  @pulling-down="onPullingDown" @pulling-up="onPullingUp" >
              ...
              <div class="dlsdetailbox" v-if="inde==2">
                <div class="left" @click="goWatchLeader(list)" v-for="(list,index) in goodsListArry" :key="index">
  ...
  methods: {
    sendRequestxhr () {
      if (this.selectInde === 0) {
        this.getjhxjList() // 进货下级列表
      } else if (this.selectInde === 1) {
        this.getxjtdList() // 下级团队列表
      } else if (this.selectInde === 2) {
        this.getjhleaderList()
      }
    },
    goshowMore (inde) {
    ...
    searchstart () {
      ...
      this.$router.push({name: 'teamMsg', query: {key: this.key}})
      this.key = ''
    },
    checkTeam (id) {
    ...
    getjhxjList () {
      var that = this
      this.$http.get('purchaseSubordinate').then((res) => {
        if (res) {
    ...
    getjhleaderList (subAgentId) {
      this.$http.get('purchaseSuperior').then((res) => {
        if (res) {
          this.tabLabels[this.selectInde].xhrFlag = true
          this.tabLabels[this.selectInde].hasNexPage = false
          var currentArry = [].concat(res.data.data.purchaseSuperior)
          this.AllGoodsListArry[this.selectInde] = currentArry
          this.$forceUpdate() // 强制刷新一下页面
        }
      }, () => {})
